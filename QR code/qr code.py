...
#folder to save all the codes
    ...
    try:
        if len(name.get()) != 0:
            ...
            qrImage = myQr.png(os.path.join(dir, nane.get() + ".png"), scale=6)
...
subjectEntry.grid(row=0, column=1, stick=N + S + E + W)
...
# Making responsive layout: totalRows = 3
...
for col in range (totalCols + 1):
    window.grid_columnconfigure(col, weight=1) #looping the GUI
